#to run: python /home/user/Documents/Tim/NLP/final_project/tim_q4/SieBERT/SieBERT_v6_attention_optimised.py
...
CONFIG = {
    'model': {
        ...
        'attention': {
            'num_sentences': 4,  # Increased attention window
            'temperature': 0.1,  # Controls attention sharpness
            'use_layer_norm': True  # Added layer normalization
        }
    },
    'training': {
        'seed': 42,
...
class LastSentenceAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        attention_weights = self.attention(hidden_states).squeeze(-1) / self.temperature
        
        # Apply mask and softmax
        attention_weights = attention_weights.masked_fill(~attention_mask.bool(), float('-inf'))
        attention_weights = torch.softmax(attention_weights, dim=1)
        
        # Create enhanced position-based weights
        seq_length = hidden_states.size(1)
        position_weights = torch.arange(seq_length, device=hidden_states.device, dtype=torch.float32)
        
        # Smoother transition with exponential decay
        position_weights = torch.exp((position_weights - seq_length) / (self.num_sentences * 5))
        position_weights = position_weights.unsqueeze(0)
        
        # Combine attention and position weights with residual connection
        final_weights = (attention_weights * position_weights + attention_weights) / 2
        final_weights = final_weights / (final_weights.sum(dim=1, keepdim=True) + 1e-9)
        
        # Apply attention with skip connection
        weighted_output = torch.bmm(final_weights.unsqueeze(1), hidden_states).squeeze(1)
...
class SentimentClassifier(nn.Module):
    def __init__(self, config: Dict = CONFIG):
        ...
        # Add last sentence attention with config
        self.last_sentence_attention = LastSentenceAttention(hidden_size, config)
        
        # Enhanced head architecture with residual connections
        ...
        # Output block
        self.fc3 = nn.Linear(512, config['model']['num_classes'])
    ...
    def forward(self, input_ids, attention_mask):
        ...
        weighted_output, _ = self.last_sentence_attention(sequence_output, attention_mask)
        
        # First block
        x = self.drop(weighted_output)
        x = self.fc1(x)
