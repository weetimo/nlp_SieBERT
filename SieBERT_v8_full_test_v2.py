...
CONFIG = {
    ...
    'logging': {
        'log_interval': 100,
        'save_top_k': 3,
        'metrics_to_track': ['f1', 'roc_auc'],
        'log_misclassified': True
    },
    'paths': {
        'output_dir': '/home/user/Documents/Tim/NLP/final_project/tim_q4/training_runs',
        'train_pos_path': '/home/user/Documents/Tim/NLP/final_project/tim_q4/full_data/train/train_pos.csv',
        'train_neg_path': '/home/user/Documents/Tim/NLP/final_project/tim_q4/full_data/train/train_neg.csv',
        'test_pos_path': '/home/user/Documents/Tim/NLP/final_project/tim_q4/full_data/testing/test_pos.csv',
        'test_neg_path': '/home/user/Documents/Tim/NLP/final_project/tim_q4/full_data/testing/test_neg.csv'
    }
}
...
class MultiHeadLastSentenceAttention(nn.Module):
    def __init__(self, hidden_size, config):
        super().__init__()
        self.num_heads = config['model']['attention']['num_heads']
        self.head_dim = hidden_size // self.num_heads
        self.temperature = config['model']['attention']['temperature']
        self.num_sentences = config['model']['attention']['num_sentences']
        ...
        self.attention_dropout = nn.Dropout(config['model']['attention']['dropout'])
        if config['model']['attention']['use_layer_norm']:
            self.layer_norm = nn.LayerNorm(hidden_size)
        self.use_layer_norm = config['model']['attention']['use_layer_norm']
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        query = self.query(hidden_states).view(batch_size, -1, self.num_heads, self.head_dim).transpose(1, 2)
        key = self.key(hidden_states).view(batch_size, -1, self.num_heads, self.head_dim).transpose(1, 2)
        value = self.value(hidden_states).view(batch_size, -1, self.num_heads, self.head_dim).transpose(1, 2)
        
        # Scaled dot-product attention with temperature
        attention_scores = torch.matmul(query, key.transpose(-2, -1)) / (self.head_dim ** 0.5 * self.temperature)
        ...
        position_weights = torch.exp((torch.arange(seq_length, device=output.device, dtype=torch.float32) - seq_length) / (self.num_sentences * 5))
        position_weights = position_weights.unsqueeze(0).unsqueeze(-1)
        
        # Combine with residual connection
        output = output * position_weights + hidden_states
...
class SentimentDataset(Dataset):
    def __init__(
        ...
        self.texts = texts.copy()  # Create a copy to avoid modifying original data
        self.labels = labels.copy()
        self.tokenizer = tokenizer
        self.max_length = max_length
        
        # Perform augmentation during initialization if enabled
        if augment and CONFIG['data']['augmentation']['enabled']:
            ...
            for text, label in tqdm(zip(texts, labels), desc="Augmenting data", total=len(texts)):
                aug_texts = self.augmenter.augment(text)
                augmented_texts.extend(aug_texts)
                augmented_labels.extend([label] * len(aug_texts))
            
            # Extend the dataset with augmented samples
            self.texts.extend(augmented_texts)
            self.labels.extend(augmented_labels)
            
            logger.info(f"Dataset size after augmentation: {len(self.texts)} samples")
...
class SentimentTrainer:
    ...
    def train(
        ...
        if CONFIG['training']['scheduler']['type'] == 'one_cycle':
        ...
        try:
            for epoch in range(epochs):
                ...
                for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}")):
                    try:
                        ...
                        if (batch_idx + 1) % gradient_accumulation_steps == 0:
                            ...
                            elif CONFIG['training']['scheduler']['type'] == 'one_cycle':
                                lr_scheduler.step()
                            optimizer.zero_grad()
                            global_step += 1
                ...
                # Update learning rate based on validation f1 if using ReduceLROnPlateau
                if CONFIG['training']['scheduler']['type'] != 'one_cycle':
...
def main():
    ...
    wandb.init(project="NLP_SieBERT_v7_full_data", config=CONFIG)
