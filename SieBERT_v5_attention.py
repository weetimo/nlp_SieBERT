...
class LastSentenceAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, num_sentences=3):
        ...
        # Create position-based weights that favor the end
        seq_length = hidden_states.size(1)
        position_weights = torch.arange(seq_length, device=hidden_states.device, dtype=torch.float32)
        position_weights = torch.sigmoid((position_weights - (seq_length - num_sentences * 10)) / 2)
        position_weights = position_weights.unsqueeze(0)
        
        # Combine attention and position weights
        final_weights = attention_weights * position_weights
...
class SentimentClassifier(nn.Module):
    def __init__(self, n_classes: int = CONFIG['model']['num_classes']):
        ...
        # Add last sentence attention
        self.last_sentence_attention = LastSentenceAttention(hidden_size)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # Get the full sequence output
        sequence_output = outputs.last_hidden_state
        
        # Apply last sentence attention
